  ...
  try {
    ...
    if (data.output) {
      for (const item of data.output) {
        ...
        // Alternative format: inline image in content
        if (item.type === 'message' && item.content) {
          for (const part of item.content) {
            if (part.type === 'image' && part.image_url) {
              // Could be a URL or base64
              generatedImageB64 = part.image_url
              break
            }
    ...
    // Also check data.output directly for flat array of content items
    ...
    if (!generatedImageB64) {
      console.error('No generated image found. Full response structure:', JSON.stringify(data, null, 2).substring(0, 2000))
      return NextResponse.json({ 
        error: 'No image generated by OpenAI. The AI may have declined to transform this photo. Try a different photo or style.',
        debug: process.env.NODE_ENV === 'development' ? JSON.stringify(data).substring(0, 500) : undefined
      }, { status: 502 })
    }
